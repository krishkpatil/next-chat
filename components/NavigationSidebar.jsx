    ...
    <div className="w-16 bg-gray-100 border-r border-gray-200 flex flex-col items-center py-4">
      <div className="mb-8">
        <div className="w-10 h-10 bg-green-600 rounded-lg flex items-center justify-center text-white font-semibold">
          P<span className="text-xs absolute bottom-0 right-0 bg-white text-green-600 rounded-full px-1">12</span>
        </div>
      </div>
        ...
        {navItems.map((item, index) => (
          <div 
            ...
            className={`w-10 h-10 flex items-center justify-center rounded-lg cursor-pointer
              ${item.active ? 'bg-green-100 text-green-600' : 'text-gray-500 hover:bg-gray-200'}`}
          >
            {getIcon(item.icon)}
          </div>
        ))}
      ...
      <div className="mt-auto space-y-6 pt-6">
        {bottomNavItems.map((item, index) => (
          <div 
            key={index} 
            className="w-10 h-10 flex items-center justify-center text-gray-500 rounded-lg cursor-pointer hover:bg-gray-200"
          >
            {getIcon(item.icon)}
          </div>
        ))}
